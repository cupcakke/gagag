...
class ThinkParser:
    ...
    def feed(self, delta):
        ...
        while self.buffer:
            if self.state == "content":
                ...
                raise RuntimeError(f"invalid parser state: {self.state}")
    ...
    def flush(self):
        ...
        if self.state == "thinking":
            if self.buffer:
                events.append(("thinking_delta", self.buffer))
            events.append(("thinking_error", "unclosed thinking block"))
...
class TokenBudget:
    def __init__(self, total: int):
        total = int(total)
        if total < 0:
            raise ValueError("total token budget must be nonnegative")
