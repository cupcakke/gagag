    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            migrations = [
                ...
                ("events", "utility_score", "REAL DEFAULT 0.5"),
                ("events", "access_count", "INTEGER DEFAULT 0"),
                ("events", "last_access_ts", "REAL"),
                ("event_archive", "utility_score", "REAL"),
                ("event_archive", "access_count", "INTEGER"),
                ("event_archive", "last_access_ts", "REAL"),
                ("semantic", "utility_score", "REAL DEFAULT 0.5"),
                ("semantic", "access_count", "INTEGER DEFAULT 0"),
                ...
                ("idempotency_keys", "result_json", "TEXT"),
            ]
            ...
            with self.transaction(conn):
                ...
                if ("tasks", "base_priority") in added_columns:
                    conn.execute(
                        "UPDATE tasks SET base_priority = priority"
                    )
                ...
                conn.execute(
                    """
                    UPDATE events
                    SET utility_score = 0.5
                    WHERE utility_score IS NULL
                    """
                )
                conn.execute(
                    """
            ...
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_job_chunks_job_seq
                    ON job_chunks(job_id, seq);
                ...
                CREATE INDEX IF NOT EXISTS idx_events_utility
                    ON events(utility_score);
        ...
        with self._lock:
            if conn.in_transaction:
                raise sqlite3.ProgrammingError(
                    "Cannot start a transaction while the connection already has an active transaction"
                )
